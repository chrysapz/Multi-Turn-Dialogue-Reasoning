...
def train(args, train_dataset, model, tokenizer):
    ...
    for _ in train_iterator:
        ...
        if args.local_rank == -1 and args.evaluate_during_training:  # Only evaluate when single GPU otherwise metrics may not average well
            ...
            if results["R4_1"] > best_dev_acc:
                best_dev_acc = results["R4_1"]
                best_dev_lossargmax = results["eval_loss"]
                best_steps = global_step
                if args.do_test:
                    results_test = evaluate(args, model, tokenizer, test=True)
                    for key, value in results_test.items():
                        tb_writer.add_scalar('test_{}'.format(key), value, global_step)
                    logger.info("test R4_1: %s, loss: %s, global steps: %s", str(results_test['R4_1']),
                                str(results_test['eval_loss']), str(global_step))
        # tb_writer.add_scalar('lr', scheduler.get_lr()[0], global_step)
        tb_writer.add_scalar('loss', (tr_loss - logging_loss) / args.logging_steps, global_step)
        logger.info("Average loss: %s at global step: %s",
                    str((tr_loss - logging_loss) / args.logging_steps), str(global_step))
...
def evaluate(args, model, tokenizer, prefix="", test=False):
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        ...
        for i in range(len(preds)):
            ...
            mrr += 1 / (j.index(preds[i][out_label_ids[i]]) + 1)
            if j.index(preds[i][out_label_ids[i]]) == 1:
                p2 += 1
        all_mrr = mrr/len(preds)
        R4_2 = p2/len(preds)
        result = {"R4_1": acc, "R4_2:": acc + R4_2, "MRR:": all_mrr, "eval_loss": eval_loss}
...
>>>>>>> 1d6c051af455fd08f4cda48cb30569563b58a3ae
    if args.local_rank == 0:
        torch.distributed.barrier()  # Make sure only the first process in distributed training will download model & vocab
